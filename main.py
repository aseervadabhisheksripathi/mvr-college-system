# Complete MVR College Automated Call System
# main.py - Save this file on your computer
...
# ============= CONFIGURATION =============
# You'll set these as environment variables on Render.com
...
def get_google_sheet():
    ...
    try:
        ...
        return sheet.worksheet('Students')  # Main worksheet
...
def get_call_log_sheet():
    ...
    try:
        ...
        client = gspread.authorize(creds)
        sheet = client.open_by_key(SPREADSHEET_ID)
        return sheet.worksheet('CallLogs')  # Call logs worksheet
    except:
        return None
...
<body>
    <div class="container">
        ...
        <div class="info-card">
            ...
            <p>
                1. Student data is managed directly in Google Sheets - click "Open Google Sheet" to edit<br>
                2. Click "Late Call" to notify parents about late arrival (absence notification)<br>
                3. Click "Permission Call" to request parent's permission for student to leave hostel<br>
                4. All calls are in Telugu with automated voice<br>
                5. Permission calls will ask parents to press 1 (allow) or 2 (deny)
            </p>
        </div>
    ...
    <script>
        ...
        function renderTable(students) {
            if (!students || students.length === 0) {
                document.getElementById('tableContent').innerHTML = '<p class="loading">No students found. Add students in Google Sheets.</p>';
                return;
            }
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-buttons">
                                <button class="action-btn call-late" onclick="initiateCall(${index}, 'late')">
                                    Late Call
                                </button>
                                <button class="action-btn call-permission" onclick="initiateCall(${index}, 'permission')">
                                    Permission
                                </button>
                            </div>
                        </td>
        ...
        function initiateCall(index, callType) {
            ...
            // Show modal for phone selection
            ...
            info.innerHTML = `
                ...
                <p><strong>Register Number:</strong> ${student['Register Number']}</p>
            `;
            ...
            if (student['Father Phone']) {
                optionsHtml += `
                    <label>
                        <input type="radio" name="phoneSelect" value="father">
                        Call Father: ${student['Father Name']} (${student['Father Phone']})
                    </label>
                `;
            }
            if (student['Mother Phone']) {
                optionsHtml += `
                    <label>
                        <input type="radio" name="phoneSelect" value="mother">
                        Call Mother: ${student['Mother Name']} (${student['Mother Phone']})
                    </label>
                `;
            }
        ...
        function makeSelectedCall() {
            ...
            fetch(endpoint, {
                ...
                body: JSON.stringify({
                    row_index: index + 2, // Google Sheets rows start at 1, plus header
                    target: target
                })
            })
            .then(response => response.json())
        ...
        function openSheetsLink() {
            // This will be replaced with actual sheet URL
...
def index():
    """Main dashboard"""
    return render_template_string(HTML_TEMPLATE.replace('{{ spreadsheet_id }}', SPREADSHEET_ID))
...
def make_late_call():
    ...
    try:
        ...
        target = data.get('target', 'father')  # 'father' or 'mother'
        
        # Get student data from sheet
        worksheet = get_google_sheet()
        student_row = worksheet.row_values(row_index)
        
        # Map row data to fields (adjust indices based on your sheet structure)
        student_data = {
            'name': student_row[2],  # Student Name
            'gender': student_row[3],  # Gender
            'father_name': student_row[4],  # Father Name
            'mother_name': student_row[5],  # Mother Name
            'father_phone': student_row[6],  # Father Phone
            'mother_phone': student_row[7]  # Mother Phone
        }
        
        # Determine phone number and parent name
        if target == 'father':
        ...
        twiml_url = f"{request.url_root}twiml/late/{row_index}/{target}"
...
def make_permission_call():
    ...
    try:
        ...
        student_data = {
            ...
            'father_name': student_row[4],
            'mother_name': student_row[5],
            'father_phone': student_row[6],
            'mother_phone': student_row[7]
        }
        ...
        # Create TwiML URL
        twiml_url = f"{request.url_root}twiml/permission/{row_index}/{target}"
...
def twiml_late(row_index, target):
    ...
    try:
        ...
        gender = student_row[3].upper()
        parent_name = student_row[4] if target == 'father' else student_row[5]
        
        # Determine child term based on gender
        child_term = "mee abbai" if gender == 'M' else "mee ammai"
        
        # Create the message in Telugu (transliterated)
        message = f"""Namaskaram {parent_name} garu! 
        Memu MVR Engineering and Polytechnic College nunchi matladutunnamu. 
        {child_term} {student_name} college ki late ga vachinanduku, 
        college gate musiveyabadindi. 
        Andukani {child_term} ki ee roju absent veyabadutundi. 
        Gamaninchagalaru. Dhanyavadamulu!"""
        
        # Generate TwiML response
        response = VoiceResponse()
        response.say(message, voice='Polly.Aditi', language='hi-IN')  # Using Hindi voice for Telugu
...
def twiml_permission(row_index, target):
    ...
    try:
        ...
        # Determine child term
        child_term = "mee abbai" if gender == 'M' else "mee ammai"
        
        # Create the message
        message = f"""Namaskaram {parent_name} garu! 
        Memu MVR Engineering and Polytechnic College nunchi matladutunnamu. 
        {child_term} {student_name} hostel nunchi bayataki velladaniki 
        anumati adugutunnaru."""
        
        # Generate TwiML with Gather for input
        response = VoiceResponse()
